...
(defn benchmark-jblas-mmuli [n]
  (let [as (map #(DoubleMatrix/randn (pow 2 %) (pow 2 %)) (range 1 n))
    ...
    (map #((:sample-mean (criterium/quick-benchmark (.mmuli ^DoubleMatrix %2 %3 %1) {})) 0) as bs cs)))
...
(defn benchmark-vectorz-* [n]
  (let [as (map #(Matrix/createRandom (pow 2 %) (pow 2 %)) (range 1 n))
    ...
    (map #((:sample-mean (criterium/quick-benchmark (Multiplications/multiply ^Matrix %1 ^Matrix %2) {})) 0) as bs)))
