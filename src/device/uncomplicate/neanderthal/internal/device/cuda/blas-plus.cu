...
#define REAL float
    ...
    __global__ void vector_sum (const int n,
        ...
        ACCUMULATOR sum = block_reduction_sum( (gid < n) ? x[gid * stride_x] : 0.0);
        if (threadIdx.x == 0) {
            acc[blockIdx.x] = sum;
        }
    ...
    __global__ void vector_axpby (const int n,
                                  const REAL alpha, const REAL* x, const int stride_x,
                                  const REAL beta, REAL* y, int stride_y) {
    ...
    __global__ void ge_equals_no_transp (const int sd, const int fd,
        ...
        if (valid) {
            const int ia = gid_0 + gid_1 * ld_a;
            const int ib = gid_0 + gid_1 * ld_b;
            if (a[ia] != b[ib]){
                eq_flag[0]++;
            }
        }
    ...
    __global__ void ge_set (const int sd, const int fd,
                            const REAL val, REAL* a, const int ld_a) {
        const int gid_0 = blockIdx.x * blockDim.x + threadIdx.x;
        const int gid_1 = blockIdx.y * blockDim.y + threadIdx.y;
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            a[gid_0 + gid_1 * ld_a] = val;
        }
    }

    __global__ void ge_sum (const int sd, const int fd, ACCUMULATOR* acc,
                            const REAL* a, const int ld_a) {
        const int gid_0 = blockIdx.x * blockDim.x + threadIdx.x;
        const int gid_1 = blockIdx.y * blockDim.y + threadIdx.y;
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        const int i = gid_0 + gid_1 * ld_a;
    ...
    __global__ void uplo_axpby_no_transp (const int sd, const int unit, const int bottom,
                                          const REAL alpha, const REAL* a, const int ld_a,
                                          const REAL beta, REAL* b, const int ld_b) {
        const int gid_0 = blockIdx.x * blockDim.x + threadIdx.x;
        const int gid_1 = blockIdx.y * blockDim.y + threadIdx.y;
        const bool valid = (gid_0 < sd) && (gid_1 < sd);
        const bool check = valid &&
    ...
    __global__ void uplo_scal (const int sd, const int unit, const int bottom,
                               const REAL alpha, REAL* a, const int ld_a) {
        const int gid_0 = blockIdx.x * blockDim.x + threadIdx.x;
        const int gid_1 = blockIdx.y * blockDim.y + threadIdx.y;
        const bool valid = (gid_0 < sd) && (gid_1 < sd);
        const bool check = valid &&
    ...
    __global__ void tr_sum (const int sd, const int fd, const int unit, const int bottom,
                            ACCUMULATOR* acc, const REAL* a, const int ld_a) {
        const int gid_0 = blockIdx.x * blockDim.x + threadIdx.x;
        const int gid_1 = blockIdx.y * blockDim.y + threadIdx.y;
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        const bool check = valid &&
