...
#define REAL3o2 (REAL)1.5
    ...
    __global__ void vector_sqr (const int n,
                                const REAL* x, const int stride_x,
                                REAL* y, const int stride_y) {

        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            const REAL xval = x[gid * stride_x];
            y[gid * stride_y] = xval * xval;
        }
    }

    __global__ void vector_mul (const int n,
                                const REAL* x, const int stride_x,
                                const REAL* y, const int stride_y,
                                REAL* z, const int stride_z) {

        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            z[gid * stride_z] = x[gid * stride_x] * y[gid * stride_y];
        }
    }
    ...
    __global__ void vector_inv (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = (REAL)1.0 / x[gid * stride_x];
        }
    }
    ...
    __global__ void vector_abs (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(fabs)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_linear_frac (const int n,
                                        ...
                                        const REAL scaleb, const REAL shiftb,
                                        REAL* z, const int stride_z) {

        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            z[gid * stride_z] =
                (scalea * x[gid * stride_x] + shifta) /
                (scaleb * y[gid * stride_y] + shiftb);
        }
    }
    ...
    __global__ void vector_scale_shift (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = scalea * x[gid * stride_x] + shifta;
    ...
    __global__ void vector_fmod (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            z[gid * stride_z] = CAST(fmod)(x[gid * stride_x], y[gid * stride_y]);
        }
    }
    ...
    __global__ void vector_frem (const int n,
        ...
        if (gid < n) {
            z[gid * stride_z] =
                CAST(remainder)(x[gid * stride_x], y[gid * stride_y]);
        }
    }
    ...
    __global__ void vector_sqrt (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(sqrt)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_inv_sqrt (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(rsqrt)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_cbrt (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(cbrt)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_inv_cbrt (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(rcbrt)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_pow2o3 (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(pow)(x[gid * stride_x], REAL2o3);
        }
    }
    ...
    __global__ void vector_pow3o2 (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(pow)(x[gid * stride_x], REAL3o2);
        }
    }
    ...
    __global__ void vector_pow (const int n,
        ...
        if (gid < n) {
            z[gid * stride_z] =
                CAST(pow)(x[gid * stride_x], y[gid * stride_y]);
        }
    }
    ...
    __global__ void vector_powx (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(pow)(x[gid * stride_x], b);
        }
    }
    ...
    __global__ void vector_hypot (const int n,
        ...
        if (gid < n) {
            z[gid * stride_z] =
                CAST(hypot)(x[gid * stride_x], y[gid * stride_y]);
        }
    }
    ...
    __global__ void vector_exp (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(exp)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_exp2 (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(exp2)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_exp10 (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(exp10)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_expm1 (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(expm1)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_log (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(log)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_log2 (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(log2)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_log10 (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(log10)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_log1p (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(log1p)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_sin (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(sin)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_cos (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(cos)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_tan (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(tan)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_sincos (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            CAST(sincos)(x[gid * stride_x], &y[gid * stride_y], &z[gid * stride_z]);
        }
    }
    ...
    __global__ void vector_asin (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(asin)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_acos (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(acos)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_atan (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(atan)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_atan2 (const int n,
        ...
        if (gid < n) {
            z[gid * stride_z] =
                CAST(atan2)(x[gid * stride_x], y[gid * stride_y]);
        }
    }
    ...
    __global__ void vector_sinh (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(sinh)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_cosh (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(cosh)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_tanh (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(tanh)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_asinh (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(asinh)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_acosh (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(acosh)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_atanh (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(atanh)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_erf (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(erf)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_erf_inv (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(erfinv)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_erfc (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(erfc)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_erfc_inv (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(erfcinv)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_cdf_norm (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(normcdf)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_cdf_norm_inv (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(normcdfinv)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_gamma (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(tgamma)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_lgamma (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(lgamma)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_floor (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(floor)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_ceil (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(ceil)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_trunc (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(trunc)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_round (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = (REAL)CAST(lrint)(x[gid * stride_x]);
        }
    }
    ...
    __global__ void vector_modf (const int n,
        ...
        if (gid < n) {
            z[gid * stride_z] =
                CAST(modf)(x[gid * stride_x], &y[gid * stride_y]);
        }
    }
    ...
    __global__ void vector_frac (const int n,
        ...
        if (gid < n) {
            REAL dummy;
            y[gid * stride_y] = CAST(modf)(x[gid * stride_x], &dummy);
        }
    }
    ...
    __global__ void vector_fmax (const int n,
        ...
        if (gid < n) {
            z[gid * stride_z] =
                CAST(fmax)(x[gid * stride_x], y[gid * stride_y]);
        }
    }
    ...
    __global__ void vector_fmin (const int n,
        ...
        if (gid < n) {
            z[gid * stride_z] =
                CAST(fmin)(x[gid * stride_x], y[gid * stride_y]);
        }
    }
    ...
    __global__ void vector_copysign (const int n,
        ...
        if (gid < n) {
            z[gid * stride_z] =
                CAST(copysign)(x[gid * stride_x], y[gid * stride_y]);
        }
    }
    ...
    __global__ void vector_sigmoid (const int n,
        ...
        if (gid < n) {
            y[gid * stride_y] =
                CAST(tanh)((REAL)0.5 * x[gid * stride_x]) * (REAL)0.5 + (REAL)0.5;
        }
    }
    ...
    __global__ void vector_ramp (const int n,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            y[gid * stride_y] = CAST(fmax)(x[gid * stride_x], (REAL)0.0);
        }
    }
    ...
    __global__ void vector_relu (const int n, const REAL alpha,
        ...
        const int gid = blockIdx.x * blockDim.x + threadIdx.x;
        if (gid < n) {
            const REAL val = x[gid * stride_x];
            y[gid * stride_y] = CAST(fmax)(val, alpha * val);
        }
    }
    ...
    __global__ void vector_elu (const int n, const REAL alpha,
        ...
        if (gid < n) {
            const REAL val = x[gid * stride_x];
            y[gid * stride_y] = CAST(fmax)(val, alpha * expm1(val));
        }
    }


    __global__ void ge_sqr (const int sd, const int fd,
                            const REAL* a, const int ld_a,
                            REAL* b, const int ld_b) {
        const int gid_0 = blockIdx.x * blockDim.x + threadIdx.x;
        const int gid_1 = blockIdx.y * blockDim.y + threadIdx.y;
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            const REAL aval = a[gid_0 + gid_1 * ld_a];
            b[gid_0 + gid_1 * ld_b] = aval * aval;
        }
    }

    __global__ void ge_mul (const int sd, const int fd,
                            const REAL* a, const int ld_a,
                            const REAL* b, const int ld_b,
                            REAL* c, const int ld_c) {
        const int gid_0 = blockIdx.x * blockDim.x + threadIdx.x;
        const int gid_1 = blockIdx.y * blockDim.y + threadIdx.y;
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                a[gid_0 + gid_1 * ld_a] * b[gid_0 + gid_1 * ld_b];
        }
    }
    ...
    __global__ void ge_inv (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = (REAL)1.0 / a[gid_0 + gid_1 * ld_a];
        }
    }
    ...
    __global__ void ge_abs (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(fabs)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_linear_frac (const int sd, const int fd,
        ...
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                (scalea * a[gid_0 + gid_1 * ld_a] + shifta) /
                (scaleb * b[gid_0 + gid_1 * ld_b] + shiftb);
        }
    }
    ...
    __global__ void ge_scale_shift (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            c[gid_0 + gid_1 * ld_c] = scalea * a[gid_0 + gid_1 * ld_a] + shifta;
    ...
    __global__ void ge_fmod (const int sd, const int fd,
        ...
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                CAST(fmod)(a[gid_0 + gid_1 * ld_a], b[gid_0 + gid_1 * ld_b]);
        }
    }
    ...
    __global__ void ge_frem (const int sd, const int fd,
        ...
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                CAST(remainder)(a[gid_0 + gid_1 * ld_a], b[gid_0 + gid_1 * ld_b]);
        }
    }
    ...
    __global__ void ge_sqrt (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(sqrt)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_inv_sqrt (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(rsqrt)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_cbrt (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(cbrt)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_inv_cbrt (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(rcbrt)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_pow2o3 (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(pow)(a[gid_0 + gid_1 * ld_a], REAL2o3);
        }
    }
    ...
    __global__ void ge_pow3o2 (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(pow)(a[gid_0 + gid_1 * ld_a], REAL3o2);
        }
    }
    ...
    __global__ void ge_pow (const int sd, const int fd,
        ...
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                CAST(pow)(a[gid_0 + gid_1 * ld_a], b[gid_0 + gid_1 * ld_b]);
        }
    }
    ...
    __global__ void ge_powx (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            c[gid_0 + gid_1 * ld_c] = CAST(pow)(a[gid_0 + gid_1 * ld_a], b);
        }
    }
    ...
    __global__ void ge_hypot (const int sd, const int fd,
        ...
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                CAST(hypot)(a[gid_0 + gid_1 * ld_a], b[gid_0 + gid_1 * ld_b]);
        }
    }
    ...
    __global__ void ge_exp (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(exp)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_exp2 (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(exp2)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_exp10 (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(exp10)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_expm1 (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(expm1)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_log (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(log)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_log2 (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(log2)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_log10 (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(log10)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_log1p (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(log1p)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_sin (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(sin)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_cos (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(cos)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_tan (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(tan)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_sincos (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            CAST(sincos)(a[gid_0 + gid_1 * ld_a],
                         &b[gid_0 + gid_1 * ld_b], &c[gid_0 + gid_1 * ld_c]);
        }
    }
    ...
    __global__ void ge_asin (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(asin)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_acos (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(acos)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_atan (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(atan)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_atan2 (const int sd, const int fd,
        ...
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                CAST(atan2)(a[gid_0 + gid_1 * ld_a], b[gid_0 + gid_1 * ld_b]);
        }
    }
    ...
    __global__ void ge_sinh (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(sinh)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_cosh (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(cosh)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_tanh (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(tanh)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_asinh (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(asinh)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_acosh (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(acosh)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_atanh (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(atanh)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_erf (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(erf)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_erf_inv (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(erfinv)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_erfc (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(erfc)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_erfc_inv (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(erfcinv)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_cdf_norm (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(normcdf)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_cdf_norm_inv (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(normcdfinv)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_gamma (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(tgamma)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_lgamma (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(lgamma)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_floor (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(floor)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_ceil (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(ceil)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_trunc (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(trunc)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_round (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(round)(a[gid_0 + gid_1 * ld_a]);
        }
    }
    ...
    __global__ void ge_modf (const int sd, const int fd,
        ...
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                CAST(modf)(a[gid_0 + gid_1 * ld_a], &b[gid_0 + gid_1 * ld_b]);
        }
    }
    ...
    __global__ void ge_frac (const int sd, const int fd,
        ...
        if (valid) {
            REAL dummy;
            b[gid_0 + gid_1 * ld_b] = CAST(modf)(a[gid_0 + gid_1 * ld_a], &dummy);
        }
    }
    ...
    __global__ void ge_fmax (const int sd, const int fd,
        ...
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                CAST(fmax)(a[gid_0 + gid_1 * ld_a], b[gid_0 + gid_1 * ld_b]);
        }
    }
    ...
    __global__ void ge_fmin (const int sd, const int fd,
        ...
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                CAST(fmin)(a[gid_0 + gid_1 * ld_a], b[gid_0 + gid_1 * ld_b]);
        }
    }
    ...
    __global__ void ge_copysign (const int sd, const int fd,
        ...
        if (valid) {
            c[gid_0 + gid_1 * ld_c] =
                CAST(copysign)(a[gid_0 + gid_1 * ld_a], b[gid_0 + gid_1 * ld_b]);
        }
    }
    ...
    __global__ void ge_sigmoid (const int sd, const int fd,
        ...
        if (valid) {
            b[gid_0 + gid_1 * ld_b] =
                CAST(tanh)((REAL)0.5 * a[gid_0 + gid_1 * ld_a]) * (REAL)0.5 + (REAL)0.5;
        }
    }
    ...
    __global__ void ge_ramp (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            b[gid_0 + gid_1 * ld_b] = CAST(fmax)(a[gid_0 + gid_1 * ld_a], (REAL)0.0);
        }
    }
    ...
    __global__ void ge_relu (const int sd, const int fd,
        ...
        const bool valid = (gid_0 < sd) && (gid_1 < fd);
        if (valid) {
            const REAL val = a[gid_0 + gid_1 * ld_a];
            b[gid_0 + gid_1 * ld_b] = CAST(fmax)(val, alpha * val);
        }
    }
    ...
    __global__ void ge_elu (const int sd, const int fd,
        ...
        if (valid) {
            const REAL val = a[gid_0 + gid_1 * ld_a];
            b[gid_0 + gid_1 * ld_b] = CAST(fmax)(val, alpha * expm1(val));
        }
    }
