...
(ns uncomplicate.neanderthal.random-test
  (:require [midje.sweet :refer [facts throws => roughly]]
            ...
            [uncomplicate.neanderthal
             [core :refer [col row sum vctr ge]]
             [random :refer :all]])
...
(defn test-vctr-rand-uniform-device [factory]
  (facts "Test vector rand-uniform on a device."
         (let [m 99
           ...
           (with-release [seed (rng-state factory 42)
                          a (ge factory m n)]
             (/ (double (sum (rand-uniform! seed -100 100 (row a 1)))) m)
             => (roughly 0 1)
             (/ (double (sum (rand-uniform! seed 0 1 (row a 1)))) m)
             => (roughly 0.5 0.1)))))

(defn test-vctr-rand-uniform-host [factory]
  (facts "Test vector rand-uniform on the host."
         (let [m 99
               n 77]
           (with-release [seed (rng-state factory 42)
                          a (ge factory m n)]
...
(defn test-vctr-rand-normal-device [factory]
  (facts "Test vector rand-normal on a device."
         (let [m 99
           ...
           (with-release [seed (rng-state factory 42)
                          a (ge factory m n)]
             (/ (double (sum (rand-normal! seed 0 100 (row a 1)))) m)
             => (roughly 0 12)
             (/ (double (sum (rand-normal! seed 0 1 (row a 1)))) m)
             => (roughly 0 0.3)))))

(defn test-vctr-rand-normal-host [factory]
  (facts "Test vector rand-normal on the host."
         (let [m 99
               n 77]
           (with-release [seed (rng-state factory 42)
                          a (ge factory m n)]
...
(defn test-all-host [factory]
  (test-vctr-rand-uniform-host factory)
  (test-vctr-rand-normal-host factory))
